...
const Form = ({ title, onBack, onSubmitSuccess }) => {
      ...
      if (response.status === 200 && response.data.success) {
        if (onSubmitSuccess) {
          ...
          setErrorModal({ 
            ...
            message: 'Your HR request has been submitted and will be processed shortly. You will receive a confirmation email with your request details.',
            type: 'success'
          });
        }
      } else {
  ...
  const getFormSpecificFields = () => {
    const formConfigs = {
      ...
      'Other (Special) Leave': [
        <EnhancedInput formData={formData} updateField={updateField} key="leaveType" fieldName="leaveType" label="Leave Type" type="select" options={['Special Leave', 'Emergency Leave', 'Personal Leave', 'Magna Carta Leave', 'Mental Health Leave', 'Solo Parent Leave', 'Maternity Leave', 'Paternity Leave', 'Other']} required />,
        <EnhancedInput formData={formData} updateField={updateField} key="startDate" fieldName="startDate" label="Start Date" type="date" required />,
        <EnhancedInput formData={formData} updateField={updateField} key="endDate" fieldName="endDate" label="End Date" type="date" required />,
      ...
      'Requests for current leave balance status': [
        <EnhancedInput formData={formData} updateField={updateField} key="leaveType" fieldName="leaveType" label="Leave Type" type="select" options={['Sick Leave', 'Vacation Leave', 'Personal Leave', 'Magna Carta Leave', 'Mental Health Leave', 'Solo Parent Leave', 'Maternity Leave', 'Paternity Leave', 'All Types']} required />,
